...
本系统实现了局域网内多台电脑之间的GPU资源共享和AI模型推理任务的分布式执行。通过自动发现、智能调度和安全传输机制，将局域网内所有可用的GPU资源整合成一个统一的算力池，实现高效的分布式计算。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| CLUSTER_ENABLED | true | 是否启用集群功能 |
| DISCOVERY_PORT | 15300 | 发现服务端口 |
| MAIN_PORT | 15301 | 主通信端口 |
| TASK_PORT | 15302 | 任务端口 |
| DATA_PORT | 15303 | 数据端口 |
| MONITOR_PORT | 15304 | 监控端口 |
| API_PORT | 15305 | API端口 |
